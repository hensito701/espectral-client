...
- **Proof:** 20/20 consecutive ordinary launches (23:32–23:37) each wrote `Using AOT-linked classes: true` into their own `aot-<pid>.log`, with zero `[warning]`/`[error]` `[aot]` records; each launch was stopped through the engine API as soon as its proof log carried the record. Ordinary launches always pass `-Xlog:aot=info:file=aot-%p.log`, so the proof is a per-launch artifact rather than a one-off experiment. *(Caveat added 2026-09-12: this marker fires before a second resource reload caused by a suite mixin defect — see "Boot optimization research". The honest ready-at-menu median today is ~8.16 s until that defect is fixed.)*
...
**Where the time actually goes.** The JVM consumes ~52–57 CPU-seconds during a ~8 s boot (≈6.1 cores busy; CPU boosts to 3.2–3.9 GHz throughout, so the `powersave` governor is not a limiter). Of that, **`-XX:+CITime` reports 39.9 s of JIT compilation** — C1 4.4 s, C2 35.5 s — for **16 055 methods** and 2.97 MB of bytecode, against ~17 000 compilation events in `-Xlog:jit+compilation`. Garbage collection is negligible (one young collection, 4.6 ms) and I/O is served entirely from page cache (108 MB `rchar`, 0 `read_bytes`, 89 k read syscalls). **Boot is a compilation problem, not an I/O, GC, or CPU-frequency problem.** After the menu marker the process drops to 0.1–0.25 cores: the marker is a fair proxy for "boot finished" — apart from the defect below.

**Highest-value finding — an in-house defect costs ~1 s after the marker.** Every launch logs
...
`branding-mod/mc262/.../AvatarRendererMixin.java` shadows `LivingEntityRenderer#model` while targeting `AvatarRenderer`, and the shipped jar contains no refmap (verified: no `*-refmap.json` in `espectral-menu-26.2-1.3.14.jar` and no `refmap` key in the mixin config). The failed transformation aborts the in-flight resource reload, so the client runs a **second full reload**: measured 4/4 launches with 1 mixin failure, 2 `Reloading ResourceManager` and 2 `Sound engine started` records, the second marker **+960 ms** after the first (7 197 ms → 8 157 ms). A control launch with the suite jar temporarily removed shows 1 reload, 1 marker, 0 failures, 7 097 ms. Consequences: the benchmark and the launcher's own `menu_ms` both stop at the *first* marker and therefore under-report the true ready-at-menu time by ~1 s; the 3D skin-layers feature is broken on 26.2; and AOT **training** runs (which run slower under record mode) escalate the same failure into a full client crash — two of two training attempts wrote a partial cache before dying. Fixing the shadow (target the declaring superclass, or ship/repair the refmap) is worth more than every JVM lever measured below combined.

**Measured levers.**

| Lever | Measured effect | Verdict |
|---|---|---|
| Fix `AvatarRendererMixin` | +960 ms recovered real time (marker₁ 7 197 → marker₂ 8 157 ms) and the skin3d feature works again | **do first** (in-house, needs a mod build) |
...
**Second in-house defect (not boot-speed, but player-visible):** `GammaEngine` writes `options.gamma().set(15.0)` every tick while fullbright is enabled, but vanilla's gamma range is 0–1, so every launch logs 24–25 `Illegal option value 15.0 for Brightness` errors and fullbright silently does nothing.

**Ranked next actions.** 1) Fix `AvatarRendererMixin` and re-verify that a launch produces exactly one resource reload and one marker. 2) Make the gate honest: keep the current marker but assert `FAILED during APPLY == 0` and a single reload, then re-baseline the medians (the honest number today is ~8.16 s, not ~7.19 s). 3) Surface `fast_boot` as a visible, reversible Fast profile once Workstream 2 bounds its frame-time cost. 4) Fix the fullbright gamma value. 5) Watch the JDK tier: JEP 544 is the only lever that can remove the remaining ~40 s of boot-time compilation, and AOT method-profile caches (JEP 515) only help *after* boot. 6) Do not spend effort on GC flags, CPU governors, launcher prefetch, or additive boot mods — all measured or reasoned to ~zero.
